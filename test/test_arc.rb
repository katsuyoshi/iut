...
class TestArc < Test::Unit::TestCase
  ...
  def test_non_arc_case_dealloc_should_not_use_arc
    assert(@arc.nonarc?("NonArcCaseDealloc"))
  end

  def test_non_arc_case_property_copy_should_not_use_arc
    assert(@arc.nonarc?("NonArcWithPropertyCopy"))
  end

  def test_non_arc_case_property_retain_should_not_use_arc
    assert(@arc.nonarc?("NonArcWithPropertyRetain"))
  end

  def test_non_arc_with_autorelease_in_methods_should_not_use_arc
    assert(@arc.nonarc?("NonArcWithAutoreleaseInMethod"))
  end

  def test_non_arc_with_release_in_methods_should_not_use_arc
    assert(@arc.nonarc?("NonArcWithReleaseInMethod"))
  end

  def test_arc_with_retain_in_comment_should_use_arc
    assert(@arc.arc?("ArcWithRetainInComment"))
  end

  def test_arc_should_use_arc
    assert(@arc.arc?("Arc"))
  end
